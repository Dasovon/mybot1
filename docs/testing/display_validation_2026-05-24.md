...
ESP32 battery JSON (`/dev/ttyUSB0`, CH340 UART0) is read when available.
Falls back gracefully when ESP32 is not connected.
That JSON stream is implemented in Phase 6 firmware (BME680 addition).
